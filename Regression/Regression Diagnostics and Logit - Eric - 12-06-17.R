...
## Created on: 12-06-17
## Created by: Eric Novotny
## Last saved: 12-06-17
## Saved by: Eric Novotny
## Description: Checking Regression Diagnotics and Doing Logit Regression
...
##############################                                       #################################
##############################           2 Logit 2 Quit:             #################################
############################## Diagnostics and Logit Regression in R #################################
##############################                                       #################################
##############################                                       #################################
######################################################################################################
...
##    folder with a slash (/). An example of this on a Windows (Mac may be different) would be
##    setwd("C:/Users/Eric/OneDrive/Documents/+ Fall 17/R Stuff")
...
##  Assign the DV(s) and IV(s) you want to use in your regression. Remember Logit Regression deals with a
##    dichotomous outcome variable. For more info on logit regression, see: 
##    https://stats.idre.ucla.edu/r/dae/logit-regression/
##    Type the name of your variables below where it says DV, IV1, IV2, etc.Note that you can use as many IVs as you want here.
##    If you need to add more IVs, simply add them after IV2 below. 

##  In my example, the dichotomous DV is Funyuns Opinion (0 = dislike, 1 = like). The continuous IV is height in inches.

##  Now we're ready to run our logit regression using the glm function. Here, we apply the glm function to 
##    our variables of interest and save the logit regression model as "mylogit1". The "family = binomial" 
##    part is what makes this a logit regression. This will generally look like this:

mylogit1 <- glm(DV ~ IV1, IV2, data = mydata, family = binomial)

##  And for my example, would look like this:

mylogit1 <- glm(Funyuns_Opinion ~ Height, data = mydata, family = binomial)
...
with(mydata, plot(Height, Funyuns_Opinion))
abline(mylogit1)
...
##  NOTE: Logistic regression doesn't include many assumptions of linear regression. For example:
##    1. Logistic regression doesn't need homoskedastic variance
##    2. Relationship between IV and Dv doesn't need to be linear
##    3, Does not need normally distributed error terms
##    4. IVs don't need to be multivariate normal
##    5. IVs don't have to be continuous, but can be

##  The following assumptions  DO apply to logistic regression:
##    1. Dichotomous DV
##    2. Error terms independent
##    3. IVs linearly related to LOG ODDS of DV
##    4. Good fit to model
##    5. Large sample size (10-30 cases per IV)
...
##  Even though logit doesn't require alot of these diagnostics, I will show them for demonstration purposes

mylogit1.stdres = rstandard(mylogit1)
...
qqnorm(mylogit1.stdres, ylab="Standardised Residuals", 
       xlab="Normal Scores", main="Funyuns_Opinion by Height") 
qqline(mylogit1.stdres) 
...
with(mydata, plot(density(Funyuns_Opinion)))


## Now try a scale-location plot. This plot uses the square root of the standardized residuals. There 
##    should be no discernable pattern to the plot (because randomness is an indication of linearity).
##    Using plot(mylogi1), you can see check for outliers. This is the gold-standrd of diagnostics.

plot(mylogit1$fitted.values, mylogit1.stdres, 
